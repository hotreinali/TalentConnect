...
public class ResumeServiceImpl implements ResumeService {
    ...
    public String uploadResume(MultipartFile file, String jobSeekerId) {
        try {
            ...
            String signatureBase = "access_mode=public&public_id=" + publicId + "&timestamp=" + timestamp
                    + "&type=upload";
            String signature = DigestUtils.sha1Hex(signatureBase + apiSecret);

            HttpPost post = new HttpPost(uploadUrl);
            MultipartEntityBuilder builder = MultipartEntityBuilder.create();
            builder.addBinaryBody("file", file.getInputStream(), ContentType.APPLICATION_OCTET_STREAM,
            ...
            builder.addTextBody("resource_type", "auto");
            builder.addTextBody("access_mode", "public");
